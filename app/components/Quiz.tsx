...
export default function Quiz({ topic, section, quizTitle, quizData }: QuizProps) {
  ...
  const getUnansweredPages = useCallback((): number[] => {
    const pages: number[] = [];
    const mcUnanswered = multipleChoiceQuestions.some((q) => !(mcAnswers[q.id] || "").trim());
    const idUnanswered = identificationQuestions.some((q) => !(idAnswers[q.id] || "").trim());
    const enumUnanswered = programmingSection ? false : enumerationQuestions.some((q) => !(enumAnswers[q.id] || "").trim());
    if (mcUnanswered) pages.push(0);
    if (idUnanswered) pages.push(1);
    if (enumUnanswered) pages.push(2);
    return pages;
  }, [mcAnswers, idAnswers, enumAnswers, enumerationQuestions, programmingSection]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (unansweredPages.length > 0) {
      ...
      const partNames = ["Multiple Choice", "Identification", programmingSection ? "Programming" : "Enumeration"];
      setSubmitError(`Please answer all questions. You have unanswered items in Part ${firstPage + 1}: ${partNames[firstPage]}.`);
      return;
    }
    gradeQuiz();
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-slate-100 p-6 md:p-10">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-6">
          ...
          <p className="text-slate-500 text-sm mt-1">Section {section} · Page {currentPage + 1} of {totalPages}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="rounded-2xl bg-slate-800/60 border border-slate-600/50 p-6 md:p-8 shadow-2xl">
            ...
            {currentPage === 2 && programmingSection ? (
              <>
